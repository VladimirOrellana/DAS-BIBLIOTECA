...
namespace DAS_BIBLIOTECA
{
    public partial class MainForm : Form
    {
        ...
        private void ActualizarListaPrestamos()
        {
            ...
            var prestamosView = prestamos.Select(p => new
            {
                Id = p.Id, // Asegúrate de incluir el ID para poder identificar los préstamos
                NombreUsuario = usuarios.FirstOrDefault(u => u.Id == p.UsuarioId)?.Nombre ?? "Desconocido",
                TituloLibro = libros.FirstOrDefault(l => l.Id == p.LibroId)?.Titulo ?? "Desconocido",
                FechaPrestamo = p.FechaPrestamo,
                FechaDevolucion = p.FechaDevolucion,
                Estado = p.Devuelto ? "Entregado" : "No entregado"
            }).ToList();

            dataGridViewLibros.DataSource = prestamosView;
